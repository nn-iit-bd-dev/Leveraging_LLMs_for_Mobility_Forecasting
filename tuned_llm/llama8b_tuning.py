...
# Updated with proper train/validation/test split
...
def evaluate_model(model, tokenizer, test_df, outdir, city):
...
def main():
    ...
    results_root = Path("Result_llama_output")
    results_root.mkdir(exist_ok=True)
    
    city_dir = results_root / args.city.lower().replace(" ", "_")
    city_dir.mkdir(exist_ok=True)
    
    run_name = f"llama8b_run_{timestamp}"
    ...
    if args.evaluate_only:
        ...
        evaluate_model(model, tokenizer, test_df, outdir, args.city)
        return
    ...
    evaluation_metrics = evaluate_model(trainer.model, tokenizer, test_df, outdir, args.city)
