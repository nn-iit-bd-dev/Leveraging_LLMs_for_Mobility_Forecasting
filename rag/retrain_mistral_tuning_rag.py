#!/usr/bin/env python3
# Fine-tune Mistral 7B for hurricane impact forecasting using PEFT/LoRA with RAG context
...
def setup_model_and_tokenizer(model_name, use_8bit=False):
    """Setup quantized Mistral model and tokenizer with LoRA"""
...
def evaluate_rag_model(model, tokenizer, test_df, outdir, city):
    """Evaluate RAG-enhanced fine-tuned Mistral model on test set"""
    from transformers import pipeline
    
    print(f"Evaluating RAG Mistral model on {len(test_df)} test samples...")
...
def main():
    ...
    results_root = Path("Result_mistral_rag_output")
    results_root.mkdir(exist_ok=True)
    
    city_dir = results_root / args.city.lower().replace(" ", "_")
    city_dir.mkdir(exist_ok=True)
    
    run_name = f"mistral7b_rag_run_{timestamp}"
    ...
    print(f"=== RAG-Enhanced Hurricane Forecasting Fine-tuning with Mistral ===")
    ...
    print(f"  Validation: {len(val_texts)} samples")
    print(f"  Test: {len(test_texts)} samples")
    print(f"RAG Mistral training completed successfully!")
